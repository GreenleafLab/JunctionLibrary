...
for i, params in enumerate(itertools.product(*[expt_params.loc[:,name].dropna() for name in expt_params])):
    ...
    if len(junction.sequences) >= numCores:
        ...
        allSeqSub = [hjh.tecto_assemble.findTecto(params, junction, seq_params, args.junction_length, junction.sequences.index.tolist())] 
...
allSeqSub = workerPool.map(f, [allSeqs.loc[index] for index in indices])
workerPool.close(); workerPool.join()
...
with open(args.out_file+'.pkl', 'wb') as output:
    for loc in allSeqs.index.tolist():
